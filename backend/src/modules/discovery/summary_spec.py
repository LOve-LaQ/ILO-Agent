...
def build_summary_prompt(items, kind: str = "repo") -> str:
    ...
    quality_rules = "\n语言要求：除 category 外，所有字段必须用简体中文书写。输入里的英文描述/标题只是素材，必须阅读理解后**改写成中文**，严禁原样照抄或直接搬运英文句子。仅专有名词与业界通用技术术语（如 Kubernetes、Rust、Docker、API、RAG）可保留英文，其余一律中文化；只要出现成句的英文，就视为不合格。\n\n事实边界要求：只能依据素材里给出的标题、描述、语言、topics 来写。素材没提到的功能、性能数字、对比结论、适用场景一律不许写，也不要把“常识推断”当素材事实来写（例如把 “C/C++” 推成“CPU 原生加速”、把 “workflow” 推成“可视化编排”、给只列出部分模型的仓库补上没提到的模型名）。当素材不足以支撑 problem/highlights 时，就写保守或留空，宁可少写也不要编造。\n\n写作要求：summary 原则上控制在 40~90 个中文字符之间（按中文字数计）；素材确实单薄时允许更短，但任何情况下都不得为凑字数而添加材料外信息。如果内容较多，请拆成 2 个短句并尽早使用句号收束，切忌写成一句超过 90 字的超长单句，也不要用分号硬凑；每一条文案语意完整、自然收尾，禁止用省略号或写到一半截断；禁止‘这是一个/该项目/本文’式的空洞开头。summary 会被用于向量化语义检索，请自然融入所属领域与技术关键词（如 RAG、向量数据库、k8s、编译器），不要写泛泛的套话。"
...
- summary: 完整中文简介，40~90 字，用 1~2 句讲清“文章主题/讲了什么 + 核心观点或价值”；只写素材支持的内容，素材没说的价值/人群不要强凑
- one_liner: 一句话主题概括，不超过 40 字，语意完整
- problem: 文章讨论的核心问题/观点，不超过 45 字；素材明确给出时才写，否则留空
- tech_stack: 文章涉及的核心技术/关键词数组（2~5 个）
- highlights: 核心干货/亮点数组（2~3 个，每个不超过 18 字）
- use_cases: 适合谁看/典型场景数组（0~2 个）；素材明确支持时才写，否则给空数组"""
        subject = "文章"
    else:
        head = f"你是技术情报整理助手，负责把 GitHub 仓库整理成结构化的中文技术卡片。\n\n下面有 {len(items)} 个仓库。请为每个仓库输出一个 JSON 对象，字段如下："
        schema_desc = f"""- category: 从 [{categories}] 里选最合适的一个
- summary: 完整中文简介，40~90 字，用 1~2 句讲清“这是什么 + 主要能做什么”；只写素材支持的内容，素材没说的价值/适用场景不要强凑
- one_liner: 一句话定位，不超过 40 字，语意完整不截断
- problem: 解决什么问题/痛点，不超过 45 字；素材明确提到时才写，否则留空
- tech_stack: 核心技术栈数组（2~5 个，如编程语言、框架、关键依赖）
- highlights: 核心亮点数组（2~3 个，每个不超过 18 字）；只列素材里出现的能力
- use_cases: 典型适用场景数组（0~2 个）；素材明确支持时才写，否则给空数组"""
        subject = "仓库"
...
2. 用 Markdown 组织，总长 {DIGEST_MIN_CHARS}~{DIGEST_MAX_CHARS} 个中文字符：开头一段总述（这是什么、能做什么）；随后**按素材实际情况**选写 `### 它解决什么问题`、`### 核心能力`（无序列表，每项一句话）、`### 适合谁 / 典型场景`——某节素材不足就整节省略，不要为凑结构而编造。
3. **只基于给定材料（最重要）**。材料没提到的功能、性能数字、对比结论、适用场景、竞品对比一律不许写，也不要把「行业常识 / 对同类项目的印象」当成材料事实——例如材料没写「会话存储/排行榜」「替代 Node.js」「与 Electron 对比」「兼容 pip 配置」就一律不能出现。信息不足的小节就整节省略，宁可短也不要编造。
